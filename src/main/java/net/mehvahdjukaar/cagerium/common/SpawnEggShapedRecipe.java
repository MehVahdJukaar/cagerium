...
public class SpawnEggShapedRecipe implements CraftingRecipe, IShapedRecipe<CraftingContainer> {
    ...
    public RecipeSerializer<?> getSerializer() {
        return Cagerium.UPGRADE_RECIPE.get();
    }
    ...
    public static class Serializer extends ForgeRegistryEntry<RecipeSerializer<?>> implements RecipeSerializer<SpawnEggShapedRecipe> {
        ...
        public SpawnEggShapedRecipe fromJson(ResourceLocation pRecipeId, JsonObject pJson) {
            ...
            String[] astring = SpawnEggShapedRecipe.shrink(SpawnEggShapedRecipe.patternFromJson(GsonHelper.getAsJsonArray(pJson, "pattern")));
            int i = astring[0].length();
            int j = astring.length;
            NonNullList<Ingredient> nonnulllist = SpawnEggShapedRecipe.dissolvePattern(astring, map, i, j);
            ItemStack itemstack = SpawnEggShapedRecipe.itemStackFromJson(GsonHelper.getAsJsonObject(pJson, "result"));
